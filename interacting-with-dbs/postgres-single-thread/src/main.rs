...
fn main() -> Result<(), Error> {
    let matches = App::new(crate_name!())
        ...
        .subcommand(SubCommand::with_name(CMD_LIST).about("Print a list of users"))
        .get_matches();
    ...
    match matches.subcommand() {
        (CMD_CREATE, _) => create_table(&conn)?,
        (CMD_ADD, Some(matches)) => create_user(
            &conn,
