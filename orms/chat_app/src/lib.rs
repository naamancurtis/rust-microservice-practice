...
impl Api {
    pub fn connect() -> Result<Self, Error> {
        let database_url = dotenv::var("DATABASE_URL")
            .unwrap_or("postgres://postgres:mysecretpassword@localhost:5432".to_string());
...
mod tests {
    ...
    fn create_users() {
        ...
        let user_1 = api.register_user("user1@test.com").unwrap();
        let user_2 = api.register_user("user2@test.com").unwrap();

        let channel = api
            .create_channel(user_1.id, "Test Channel", false)
            .unwrap();
        ...
        let message = api
            .add_message(channel.id, user_1.id, "Test Message")
            .unwrap();
        api.add_message(channel.id, user_2.id, "Test").unwrap();
